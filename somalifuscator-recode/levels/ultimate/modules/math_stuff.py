...
def generate_math_problem(answer: int):
    ...
    choices = [True, False]
    problem2 = f"{random_oct_hex(ans) if random.choice(choices) else make_xor(ans)} {opp1} {random_oct_hex(num1) if random.choice(choices) else make_xor(num1)} {opp2} {random_oct_hex(num2) if random.choice(choices) else make_xor(num2)}"
    ...
    while ans < 0:
        ...
        problem2 = (
            f"{hex(ans)} {opp1} {random_oct_hex(num1)} {opp2} {random_oct_hex(num2)}"
        )
        problem23 = f"{ans} {opp1} {num1} {opp2} {num2}"
...
def make_xor(number: int, hex_check: bool = True):
    """makes xor key"""
    ans = number
    ...
    if hex_check:
        ...
        try:
            if random_tf != 1:
                ...
                return f"({make_xor(random2)} ^^ {make_xor(fixed2)})"
